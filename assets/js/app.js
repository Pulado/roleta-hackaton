...
function buildBettingBoard(){
	...
	let otoBlocks = ['EVEN', '‎ ', '‎‎', 'ODD'];
	for(i = 0; i < otoBlocks.length; i++){
		let d = i;
		var colourClass = (otoBlocks[i] == '‎ ')? ' redNum' : ((otoBlocks[i] == '‎‎')? ' blackNum' : '');
	...
	let chipValues = [1, 5, 10, 100, 'Limpar'];
		...
		notification.setAttribute('id', 'notification');
			...
			nSpan.setAttribute('class', 'nSpan');
				...
				nsWin.setAttribute('class', 'nsWin');
					...
					nsWinBlock.innerText = 'Apostou: ' + betTotal;
					nSpan.append(nsWinBlock);
					nsWin.append(nsWinBlock);
					nsWinBlock = document.createElement('div');
					nsWinBlock.setAttribute('class', 'nsWinBlock');
					nsWinBlock.innerText = 'Ganhou: ' + winValue;
					nSpan.append(nsWinBlock);
					nsWin.append(nsWinBlock);
					nsWinBlock = document.createElement('div');
					nsWinBlock.setAttribute('class', 'nsWinBlock');
					nsWinBlock.innerText = 'Pagamento: ' + (winValue + betTotal);
					nsWin.append(nsWinBlock);
				nSpan.append(nsWin);
